...
public class TriggerUtils {
  ...
  /**
   * Compute the <code>Date</code> that is 1 second after the Nth firing of the given <code>Trigger</code>, taking the triger's associated <code>Calendar</code> into consideration. The input trigger will be cloned before any work is done, so you need
  ...
  /**
   * Returns a list of Dates that are the next fire times of a <code>Trigger</code> that fall within the given date range. The input trigger will be cloned before any work is done, so you need not worry about its state being altered by this method.
   * <p>
