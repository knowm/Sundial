 ...
 * </p>
 * <p>
 * Plugins can do virtually anything you wish, though the most interesting ones will obviously interact with the scheduler in some way - either actively: by invoking actions on the scheduler, or passively: by being a <code>JobListener</code>,
 * <code>TriggerListener</code>, and/or <code>SchedulerListener</code>.
 * </p>
 * <p>
 * If you use <code>{@link org.quartz.core.SchedulerFactory}</code> to initialize your Scheduler, it can also create and initialize your plugins - look at the configuration docs for details.
 * </p>
 * <p>
...
public interface SchedulerPlugin {
   ...
   * </p>
   * <p>
   * If you need direct access your plugin, for example during <code>Job</code> execution, you can have this method explicitly put a reference to this plugin in the <code>Scheduler</code>'s <code>SchedulerContext</code>.
   * </p>
   * 
  ...
  /**
   * <p>
   * Called when the associated <code>Scheduler</code> is started, in order to let the plug-in know it can now make calls into the scheduler if it needs to.
   * </p>
   */
  void start();
