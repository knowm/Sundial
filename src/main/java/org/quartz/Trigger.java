...
public interface Trigger extends Serializable, Cloneable, Comparable<Trigger> {
   ...
   * <code>{@link org.quartz.JobDetail}</code> to re-execute immediately. If not in a 'RECOVERING' or 'FAILED_OVER' situation, the execution context
   ...
   * <code>{@link org.quartz.JobDetail}</code> as this one should be put in the <code>COMPLETE</code> state.
   * </p>
   * <p>
   * <code>SET_TRIGGER_ERROR</code> Instructs the <code>{@link Scheduler}</code> that all <code>Trigger</code>s referencing the same
   * <code>{@link org.quartz.JobDetail}</code> as this one should be put in the <code>ERROR</code> state.
   * </p>
   * <p>
