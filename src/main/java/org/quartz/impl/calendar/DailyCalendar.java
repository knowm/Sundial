 ...
 * times in which triggers are not allowed to fire. If <CODE>invertTimeRange</CODE> is <CODE>true</CODE>, the time range is inverted &ndash; that is, all times <I>outside</I> the defined time range are excluded.
 * <P>
 * Note when using <CODE>DailyCalendar</CODE>, it behaves on the same principals as, for example, {@link org.quartz.impl.calendar.WeeklyCalendar WeeklyCalendar}. <CODE>WeeklyCalendar</CODE> defines a set of days that are excluded <I>every week</I>.
 * Likewise, <CODE>DailyCalendar</CODE> defines a set of times that are excluded <I>every day</I>.
 * 
...
public class DailyCalendar extends BaseCalendar {
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified strings and no <CODE>baseCalendar</CODE>. <CODE>rangeStartingTime</CODE> and <CODE>rangeEndingTime</CODE> must be in the format &quot;HH:MM[:SS[:mmm]]&quot; where:
   * <UL>
   * <LI>HH is the hour of the specified time. The hour should be specified using military (24-hour) time and must be in the range 0 to 23.</LI>
   ...
   * <LI>The time range starting time must be before the time range ending time. Note this means that a time range may not cross daily boundaries (10PM - 2AM)</LI>
   * </UL>
   * <p>
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code>
   * </p>
   * 
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified strings and the specified <CODE>baseCalendar</CODE>. <CODE>rangeStartingTime</CODE> and <CODE>rangeEndingTime</CODE> must be in the format
   * &quot;HH:MM[:SS[:mmm]]&quot; where:
   * <UL>
   * <LI>HH is the hour of the specified time. The hour should be specified using military (24-hour) time and must be in the range 0 to 23.</LI>
   * <LI>MM is the minute of the specified time and must be in the range 0 to 59.</LI>
   ...
   * </p>
   * 
   * @param baseCalendar the base calendar for this calendar instance &ndash; see {@link BaseCalendar} for more information on base calendar functionality
   * @param rangeStartingTime a String representing the starting time for the time range
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and no <CODE>baseCalendar</CODE>. Values are subject to the following validations:
   * <UL>
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and the specified <CODE>baseCalendar</CODE>. Values are subject to the following validations:
   * <UL>
   * <LI>Hours must be in the range 0-23 and are expressed using military (24-hour) time.</LI>
   * <LI>Minutes must be in the range 0-59</LI>
   * <LI>Seconds must be in the range 0-59</LI>
   ...
   * @param rangeEndingMillis the millisecond of the start of the time range
   */
  public DailyCalendar(org.quartz.Calendar baseCalendar, int rangeStartingHourOfDay, int rangeStartingMinute, int rangeStartingSecond, int rangeStartingMillis, int rangeEndingHourOfDay, int rangeEndingMinute,
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified <CODE>java.util.Calendar</CODE>s and no <CODE>baseCalendar</CODE>. The Calendars are subject to the following considerations:
   * <UL>
   * <LI>Only the time-of-day fields of the specified Calendars will be used (the date fields will be ignored)</LI>
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range may not cross daily boundaries (10PM - 2AM). <I>(because only time fields are are used, it is possible for two Calendars to
   * represent a valid time range and <CODE>rangeStartingCalendar.after(rangeEndingCalendar) == 
   *         true</CODE>)</I></LI>
   * </UL>
   * <p>
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code>
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified <CODE>java.util.Calendar</CODE>s and the specified <CODE>baseCalendar</CODE>. The Calendars are subject to the following considerations:
   * <UL>
   * <LI>Only the time-of-day fields of the specified Calendars will be used (the date fields will be ignored)</LI>
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range may not cross daily boundaries (10PM - 2AM). <I>(because only time fields are are used, it is possible for two Calendars to
   * represent a valid time range and <CODE>rangeStartingCalendar.after(rangeEndingCalendar) == 
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and no <CODE>baseCalendar</CODE>. The values are subject to the following considerations:
   * <UL>
   * <LI>Only the time-of-day portion of the specified values will be used</LI>
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range may not cross daily boundaries (10PM - 2AM). <I>(because only time value are are used, it is possible for the two values to
   * represent a valid time range and <CODE>rangeStartingTime &gt; 
   *         rangeEndingTime</CODE>)</I></LI>
   * </UL>
   * <p>
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code>. You should use
   * <code>{@link #DailyCalendar(String, TimeZone, long, long)}</code> if you don't want the given <code>rangeStartingTimeInMillis</code> and <code>rangeEndingTimeInMillis</code> to be evaluated in the default time zone.
   * </p>
   * 
  ...
  /**
   * Create a <CODE>DailyCalendar</CODE> with a time range defined by the specified values and the specified <CODE>baseCalendar</CODE>. The values are subject to the following considerations:
   * <UL>
   * <LI>Only the time-of-day portion of the specified values will be used</LI>
   * <LI>The starting time must be before the ending time of the defined time range. Note this means that a time range may not cross daily boundaries (10PM - 2AM). <I>(because only time value are are used, it is possible for the two values to
   * represent a valid time range and <CODE>rangeStartingTime &gt; 
   ...
   * <p>
   * <b>Note:</b> This <CODE>DailyCalendar</CODE> will use the <code>{@link TimeZone#getDefault()}</code> time zone unless an explicit time zone is set via <code>{@link BaseCalendar#setTimeZone(TimeZone)}</code>. You should use
   * <code>{@link #DailyCalendar(String, Calendar, TimeZone, long, long)}</code> if you don't want the given <code>rangeStartingTimeInMillis</code> and <code>rangeEndingTimeInMillis</code> to be evaluated in the default time zone.
   * </p>
   * 
   * @param baseCalendar the base calendar for this calendar instance &ndash; see {@link BaseCalendar} for more information on base calendar functionality
   * @param rangeStartingTimeInMillis a long representing the starting time for the time range
   ...
   * </UL>
   * 
   * @param timeZone the time zone for of the <code>DailyCalendar</code> which will also be used to resolve the given start/end times.
   * @param rangeStartingTimeInMillis a long representing the starting time for the time range
   * @param rangeEndingTimeInMillis a long representing the ending time for the time range
   */
   ...
   * @param rangeEndingMillis the millisecond of the start of the time range
   */
  public void setTimeRange(int rangeStartingHourOfDay, int rangeStartingMinute, int rangeStartingSecond, int rangeStartingMillis, int rangeEndingHourOfDay, int rangeEndingMinute, int rangeEndingSecond,
      int rangeEndingMillis) {
    ...
    if (!startCal.before(endCal)) {
      throw new IllegalArgumentException(invalidTimeRange + rangeStartingHourOfDay + ":" + rangeStartingMinute + ":" + rangeStartingSecond + ":" + rangeStartingMillis + separator + rangeEndingHourOfDay + ":"
  ...
  public void setTimeRange(Calendar rangeStartingCalendar, Calendar rangeEndingCalendar) {
    ...
    setTimeRange(rangeStartingCalendar.get(Calendar.HOUR_OF_DAY), rangeStartingCalendar.get(Calendar.MINUTE), rangeStartingCalendar.get(Calendar.SECOND), rangeStartingCalendar.get(Calendar.MILLISECOND),
        rangeEndingCalendar.get(Calendar.HOUR_OF_DAY), rangeEndingCalendar.get(Calendar.MINUTE), rangeEndingCalendar.get(Calendar.SECOND), rangeEndingCalendar.get(Calendar.MILLISECOND));
  }
