...
/**
 * <p>
 * A concrete <code>{@link Trigger}</code> that is used to fire a <code>{@link org.quartz.jobs.JobDetail}</code> based upon repeating calendar time
 * intervals.
 * </p>
 * <p>
 * The trigger will fire every N (see {@link #setRepeatInterval(int)} ) units of calendar time (see {@link #setRepeatIntervalUnit(IntervalUnit)}) as
 * specified in the trigger's definition. This trigger can achieve schedules that are not possible with {@link SimpleTrigger} (e.g because months are
 * not a fixed number of seconds) or {@link CronTrigger} (e.g. because "every 5 months" is not an even divisor of 12).
 * </p>
 * <p>
 * If you use an interval unit of <code>MONTH</code> then care should be taken when setting a <code>startTime</code> value that is on a day near the
 * end of the month. For example, if you choose a start time that occurs on January 31st, and have a trigger with unit <code>MONTH</code> and interval
 * <code>1</code>, then the next fire time will be February 28th, and the next time after that will be March 28th - and essentially each subsequent
 * firing will occur on the 28th of the month, even if a 31st day exists. If you want a trigger that always fires on the last day of the month -
...
public class CalendarIntervalTriggerImpl extends AbstractTrigger implements CalendarIntervalTrigger {
  ...
  /**
   * <p>
   * set the the time interval that will be added to the <code>DateIntervalTrigger</code>'s fire time (in the set repeat interval unit) in order to
   * calculate the time of the next trigger repeat.
   * </p>
   *
  ...
  /**
   * <p>
   * Updates the <code>DateIntervalTrigger</code>'s state based on the MISFIRE_INSTRUCTION_XXX that was selected when the
  ...
  /**
   * <p>
   * Returns the previous time at which the <code>DateIntervalTrigger</code> fired. If the trigger has not yet fired, <code>null</code> will be
   * returned.
   */
  @Override
  ...
  /**
   * <p>
   * Returns the next time at which the <code>DateIntervalTrigger</code> will fire, after the given time. If the trigger will not fire after the given
  ...
  /**
   * <p>
   * Returns the final time at which the <code>DateIntervalTrigger</code> will fire, if there is no end time set, null will be returned.
   * </p>
   * <p>
