...
/**
 * <p>
 * The interface to be implemented by classes that want to provide a <code>{@link org.quartz.Job}</code> and <code>{@link org.quartz.Trigger}</code>
 * storage mechanism for the <code>{@link org.quartz.core.QuartzScheduler}</code>'s use.
 * </p>
 * <p>
...
public interface JobStore {
  ...
  /**
   * Remove (delete) the <code>{@link org.quartz.Trigger}</code> with the given key, and store the new given one - which must be associated with the
   * same job.
   *
  ...
  /**
   * Get the keys of all of the <code>{@link org.quartz.Job}</code> s
   * <p>
   ...
   * associated <code>Job</code> completed, threw an exception, or was vetoed), and that the <code>{@link org.quartz.JobDataMap}</code> in the given
