...
/**
 * <p>
 * The interface to be implemented by classes that want to provide a thread pool for the <code>{@link org.quartz.core.QuartzScheduler}</code>'s use.
 * </p>
 * <p>
 * <code>ThreadPool</code> implementation instances should ideally be made for the sole use of Quartz. Most importantly, when the method <code>blockForAvailableThreads()</code> returns a value of 1 or greater, there must still be at least one
 * available thread in the pool when the method <code>runInThread(Runnable)</code> is called a few moments (or many moments) later. If this assumption does not hold true, it may result in extra JobStore queries and updates, and if clustering features
...
public interface ThreadPool {
   ...
   * </p>
   * <p>
   * The implementation of this interface should not throw exceptions unless there is a serious problem (i.e. a serious misconfiguration). If there are no immediately available threads <code>false</code> should be returned.
   * </p>
   * 
  ...
  /**
   * <p>
   * Inform the <code>ThreadPool</code> of the Scheduler instance's name, prior to initialize being invoked.
   * </p>
   * 
   * @since 1.7
   */
